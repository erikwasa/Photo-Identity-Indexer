...
{
    IReadOnlyDictionary<string, CenterFaceTensor> Run(float[] input, long[] shape, CancellationToken cancellationToken);
}
...
internal sealed class OpenCvDnnCenterFaceInferenceSession : ICenterFaceInferenceSession
{
    ...
    public IReadOnlyDictionary<string, CenterFaceTensor> Run(float[] input, long[] shape, CancellationToken cancellationToken)
    {
        ...
        {
            throw new CenterFaceOutputException($"CenterFace input must be NCHW [1, 3, H, W], but received [{string.Join(", ", shape)}].");
        }

        int[] matShape = shape.Select(dimension => checked((int)dimension)).ToArray();
        ...
        {
            throw new CenterFaceOutputException($"CenterFace input shape declares {expectedElements} values but received {input.LongLength}.");
        }

        using Mat inputBlob = Mat.FromPixelData(matShape, MatType.CV_32FC1, input);

        // Real smoke evidence and an independent CenterFace adapter both show
        // corrupted later calls when one OpenCV Net is reused. Isolate native
        // network state per image without changing candidate parameters.
    ...
    internal static CenterFaceTensor ReadOutputTensor(string name, Mat output)
    {
        ...
        {
            throw new CenterFaceOutputException($"CenterFace output '{name}' must contain float32 values, but OpenCV returned {output.Type()}.");
        }

        long[] outputShape = Enumerable.Range(0, output.Dims).Select(dimension => (long)output.Size(dimension)).ToArray();
        long elementCount = 1;
        foreach (long dimension in outputShape)
        {
            if (dimension <= 0)
            {
                throw new CenterFaceOutputException($"CenterFace output '{name}' has invalid shape [{string.Join(", ", outputShape)}].");
            }
        ...
        {
            throw new CenterFaceOutputException($"CenterFace output '{name}' contains too many elements to copy safely: {elementCount}.");
        }
    ...
    public void Dispose()
    {
        // Net instances are intentionally scoped to individual Run calls.
    }
}
