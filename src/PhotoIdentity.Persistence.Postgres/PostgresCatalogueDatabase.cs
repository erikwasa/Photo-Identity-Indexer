...
public sealed partial class PostgresCatalogueDatabase : IAsyncDisposable, ICatalogueStoreInitializer
{
    ...
    [
        ...
        new(
            ...
            CREATE TABLE photo_tags (
                ...
                display_name text NOT NULL
                    CHECK (length(display_name) BETWEEN 1 AND 500),
            ...
            CREATE TABLE photo_tag_actions (
                ...
                tag_id bigint NOT NULL
                    REFERENCES photo_tags (id) ON DELETE RESTRICT,
                action_kind text NOT NULL
                    CHECK (action_kind IN ('add', 'remove')),
                actor text NOT NULL
                    CHECK (btrim(actor) <> ''),
                created_at_utc timestamp with time zone NOT NULL
            );
        ...
        new(
            ...
            "photo-people",
            """
            CREATE TABLE photo_person_actions (
                id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                asset_revision_id uuid NOT NULL
                REFERENCES asset_revisions (id) ON DELETE CASCADE,
                person_id uuid NOT NULL
            ...
            CREATE INDEX ix_photo_person_actions_revision_history
                ON photo_person_actions (asset_revision_id, person_id, id DESC);
            CREATE INDEX ix_photo_person_actions_person_history
        ...
        new(
            ...
            "photo-metadata",
            """
            CREATE TABLE photo_extended_metadata (
            ...
            CREATE TABLE photo_metadata_inspections (
                asset_revision_id uuid PRIMARY KEY
                REFERENCES asset_revisions (id) ON DELETE CASCADE,
                extraction_contract_version integer NOT NULL
                CHECK (extraction_contract_version > 0),
                inspected_at_utc timestamp with time zone NOT NULL
            );

            CREATE INDEX ix_photo_metadata_inspections_version
                ON photo_metadata_inspections (
        ...
        new(
            ...
            "photo-places",
            """
            CREATE TABLE photo_place_actions (
                id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                asset_revision_id uuid NOT NULL
                    REFERENCES asset_revisions (id) ON DELETE CASCADE,
                tag_id bigint NULL
                    REFERENCES photo_tags (id) ON DELETE RESTRICT,
                action_kind text NOT NULL
                    CHECK (action_kind IN ('set', 'clear')),
                source_kind text NOT NULL
                    CHECK (source_kind IN ('manual', 'automatic', 'migration')),
                provider text NULL,
                actor text NOT NULL
                    CHECK (btrim(actor) <> ''),
                created_at_utc timestamp with time zone NOT NULL,
            ...
            CREATE TABLE photo_place_migration_conflicts (
                asset_revision_id uuid PRIMARY KEY
                    REFERENCES asset_revisions (id) ON DELETE CASCADE,
                candidate_values text NOT NULL
                    CHECK (char_length(candidate_values) > 0),
