...
public sealed class SqlitePhotoCaptionRepository : IPhotoCaptionRepository
{
    private static readonly JsonSerializerOptions JsonOptions = new(JsonSerializerDefaults.Web);
    private readonly SqliteCatalogueDatabase _database;

    public SqlitePhotoCaptionRepository(SqliteCatalogueDatabase database)
    {
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        string formatted = Format(updatedAtUtc);
        ...
        command.Parameters.AddWithValue("$updated_at_utc", formatted);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            result.Add(AssetRevisionId.From(Guid.Parse(reader.GetString(0))));
        }
        return result;
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        command.CommandText = """
            ...
            """;
        command.Parameters.AddWithValue("$revision_id", caption.RevisionId.ToString());
        command.Parameters.AddWithValue("$language", language);
        command.Parameters.AddWithValue("$generation_version", caption.GenerationVersion.Trim());
        command.Parameters.AddWithValue("$model_id", caption.ModelId.Trim());
        command.Parameters.AddWithValue("$model_digest", caption.ModelDigest.Trim().ToLowerInvariant());
        command.Parameters.AddWithValue("$prompt_version", caption.PromptVersion.Trim());
        command.Parameters.AddWithValue("$image_mode", caption.ImageMode.Trim());
        command.Parameters.AddWithValue("$context_tokens", caption.ContextTokens);
        command.Parameters.AddWithValue("$content", (object?)caption.Content?.Trim() ?? DBNull.Value);
        command.Parameters.AddWithValue("$risk_flags_json", JsonSerializer.Serialize(riskFlags, JsonOptions));
        command.Parameters.AddWithValue("$generation_milliseconds", caption.GenerationMilliseconds);
        command.Parameters.AddWithValue("$generated_at_utc", Format(caption.GeneratedAtUtc));
        await command.ExecuteNonQueryAsync(cancellationToken);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        string[] riskFlags = JsonSerializer.Deserialize<string[]>(
            reader.GetString(9),
            JsonOptions) ?? [];
