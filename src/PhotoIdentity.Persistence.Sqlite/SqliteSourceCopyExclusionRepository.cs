...
public sealed class SqliteSourceCopyExclusionRepository : ISourceCopyExclusionRepository
{
    private readonly SqliteCatalogueDatabase _database;
    private readonly Lazy<Task> _schema;

    public SqliteSourceCopyExclusionRepository(SqliteCatalogueDatabase database)
    {
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            command.Parameters.AddWithValue("$source_id", id.ToString());
        }
        ...
        CancellationToken cancellationToken = default)
    {
        await EnsureSchemaAsync(cancellationToken);
        string key = SourceCopyLocator.NormalizeSourceKey(sourceKey);
        string now = Format(excludedAtUtc);
        await using SqliteConnection connection = await _database.OpenConnectionAsync(cancellationToken);
        using SqliteCommand command = connection.CreateCommand();
        command.CommandText = """
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        command.Parameters.AddWithValue("$purge_error_code", (object?)errorCode ?? DBNull.Value);
        command.Parameters.AddWithValue("$updated_at_utc", Format(updatedAtUtc));
    ...
    public Task<bool> IsAssetExcludedAsync(AssetId assetId, CancellationToken cancellationToken = default) =>
        ExistsByJoinAsync("asset.id = $id", assetId.ToString(), cancellationToken);

    public Task<bool> IsRevisionExcludedAsync(AssetRevisionId revisionId, CancellationToken cancellationToken = default) =>
        ExistsByJoinAsync("revision.id = $id", revisionId.ToString(), cancellationToken);
    ...
    public Task<bool> IsFaceOccurrenceExcludedAsync(FaceOccurrenceId faceOccurrenceId, CancellationToken cancellationToken = default) =>
        ExistsByJoinAsync("face.id = $id", faceOccurrenceId.ToString(), cancellationToken);

    private async Task<bool> ExistsByJoinAsync(string predicate, string id, CancellationToken cancellationToken)
    {
        await EnsureSchemaAsync(cancellationToken);
        await using SqliteConnection connection = await _database.OpenConnectionAsync(cancellationToken);
        using SqliteCommand command = connection.CreateCommand();
