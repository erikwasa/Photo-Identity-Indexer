...
The current packaged launcher is loopback-only, while phone use necessarily arrives from another device. M21 therefore includes an explicit mobile-access prerequisite rather than silently widening the existing unauthenticated host.
