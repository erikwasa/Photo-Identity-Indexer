...
- Do not mix M22 acceptance work into this milestone.
...
5. WI-0103 and WI-0104 remove known algorithm/query/UI scaling defects on the new catalogue.
6. WI-0105 adds durable low-cost observability across the migrated runtime.
7. WI-0106 proves backup/recovery, normal startup and sustained archive catch-up, then hands off to smaller daily updates.

WI-0103 through WI-0105 may begin before final cutover when dependencies allow, but must not block safe migration progress unnecessarily.
...
- Low-overhead metrics expose background throughput, failures and key API/database timing without storing personal filenames, image content or embeddings.
