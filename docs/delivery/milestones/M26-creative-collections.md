...
WI-0128 now productizes that positive result directly as an off-by-default slideshow setting. Generated captions remain loopback-only, guarded and presentation-only. A single bounded background worker generates only photos encountered during caption-enabled playback; safe captions are cached as regenerable derived evidence so later slideshow views can reuse them. Swedish and English are selectable, with Swedish the default. M26 remains in progress until maintainer verification confirms generation/reuse does not disrupt representative slideshow playback.
