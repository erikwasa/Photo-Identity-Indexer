...
# WI-0082: Add secure trusted-LAN phone access for slideshow use
...
The slideshow's primary phone use case cannot rely on the current packaged launcher alone: it only accepts loopback HTTP URLs. In addition, browser features used by M22 such as Screen Wake Lock and installable/service-worker PWA behavior require a secure context on normal non-loopback origins.
...
Because browser/device support varies, this work item establishes the **transport/access baseline** only. WI-0085 owns capability detection for fullscreen/orientation/wake APIs and their fallbacks.
...
- [ ] Maintainer verification confirms the secure origin on the real phone that will be used for M22 acceptance.
...
- VPN setup.
- Fullscreen, orientation lock, wake lock or toddler-protection behavior; those belong to later M22 work items.
