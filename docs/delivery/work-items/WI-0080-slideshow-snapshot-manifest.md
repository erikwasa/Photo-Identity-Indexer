...
# WI-0080: Add stable Smart Collection slideshow snapshots
...
The current workspace page size is not a slideshow contract, and repeated offset queries can shift when collection membership changes. M21 requires stable playback from the collection state that existed when Start slideshow was pressed.
