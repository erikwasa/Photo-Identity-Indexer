...
- `.github/test-timing-baseline.json` is rebalanced from #1112's actual required-shard TRX data. After the 50-job change, `ResumableBatchProcessorTests` carries a conservative 20s scheduling estimate until the next measured run. The resulting two-shard plan is approximately 248.2s / 248.2s of #1112-weighted work.
