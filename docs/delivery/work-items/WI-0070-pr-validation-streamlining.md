...
- The revised Slice 2 design uses two separate Windows integration jobs, each internally sequential and independently restored/built. The #1093 weights divide to roughly half of the 216.9s recorded workload per shard. This intentionally spends additional runner setup/build minutes in exchange for real CPU/testhost isolation and a lower wall-clock critical path.
- Each shard independently discovers the full suite and computes the same deterministic plan. It then requires its TRX result count and unique test-id count to equal the number of tests assigned to that shard. The full plan is also required to account for every discovered class/test before the selected shard runs.
- The main `build-and-test` job no longer runs the integration assembly. It retains true fast tests, documentation validation, published review smoke and mixed-media verification. The integration jobs remain required PR checks, so no regression coverage is removed.
