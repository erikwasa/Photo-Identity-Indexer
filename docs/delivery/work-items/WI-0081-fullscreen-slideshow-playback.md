...
# WI-0081: Build fullscreen Smart Collection slideshow playback

## Objective

Build the core read-only slideshow experience on top of the stable WI-0080 snapshot contract: true fullscreen entry, bounded image loading, tap/swipe/keyboard navigation, autoplay and persistent global slideshow preferences.

Toddler-specific lockout and mobile capability hardening are completed by WI-0082; this item establishes the presentation state machine and controls that WI-0082 protects.
...
- After fullscreen entry, show a neutral preparing surface while WI-0080 snapshot creation completes.
...
- Touch handling must distinguish a horizontal slideshow swipe from the later WI-0082 parent-unlock gesture.
...
WI-0081 owns storage/validation for the whole settings object even though WI-0082 and WI-0083 consume the last two settings.
...
- Toddler parent-unlock/history guard/fullscreen-loss hardening beyond the basic recoverable state; WI-0082 owns it.
- Orientation lock and wake lock; WI-0082 owns them.
- Original preparation/hydration; WI-0083 owns it.
