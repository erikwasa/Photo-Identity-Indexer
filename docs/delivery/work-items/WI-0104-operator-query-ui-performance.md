...
- The final Settings slice adds `/api/archive/configuration`, backed only by `IArchiveCoverageRepository`, and splits Archive Coverage, Archive Storage and Identity Matching into independently loading components. The Settings page shell no longer awaits `/api/archive/status`, storage aggregation or review-filter/policy work before rendering archive configuration. Integration coverage replaces `IArchiveStatusRepository` with a throwing implementation and proves `/api/archive/configuration` still succeeds while full `/api/archive/status` fails, guarding the intended independence boundary.
