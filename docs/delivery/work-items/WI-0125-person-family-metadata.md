...
- [x] Automated tests cover partial dates, merges, relationship directionality/symmetry, cousin behavior and age boundary cases and pass in CI/live PostgreSQL verification.
...
The maintainer first verified that birth metadata, age filtering, the implemented relationship directions, saved/reopened family-aware Smart Collections and Creative anchor behavior worked on representative private data. The remaining requested gaps were cousin support and refinement of the People relationship presentation: repeated relationships should be grouped by category, the relation column should be wider, and the Remove action must fit/read cleanly.
...
Final maintainer verification after PR #421 confirmed the completed WI-0125 behavior works as expected, including cousin symmetry and the grouped/responsive relationship UI. PR #421 workflow run #2157 completed successfully.
...
- Implementation PRs: #415, #416, verification correction #418 and final cousin/UI follow-up #421. WI-0125 was accepted by the maintainer on 2026-09-25 and is complete.
