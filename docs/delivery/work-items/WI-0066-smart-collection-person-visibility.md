...
Slice 1 merged through PR #170. The foundation uses schema v16 with a dedicated `person_smart_collection_visibility` table and a sparse hidden-person preference. The maintenance API exposes a reversible visibility mutation plus the current flag. Merge semantics are target-wins: the surviving person's existing preference is unchanged, and preferences attached to a retired source identity are excluded from active visibility resolution.

Slice 2 merged through PR #171. Maintain People displays an explicit `Available in Smart Collections` or `Hidden from Smart Collections` status for every active person and provides a reversible hide/show action using the Slice 1 endpoint. Hidden people remain present for maintenance, rename, favorite and merge operations.

Slice 3 is implemented on `agent/WI-0066-smart-collection-discovery`. The ordinary review people response still returns every active person, but now carries the persisted `HiddenFromSmartCollections` value so Smart Collections can apply presentation-only discovery rules without changing review identity behavior. The Smart Collection people picker excludes hidden people unless their identifier is already selected by a restored saved or transient definition. Such a selected hidden person remains checked, is explicitly marked `Hidden`, and can be removed; after removal it disappears from the picker until unhidden.

Integration coverage verifies restart persistence, reversible hide/unhide behavior, continued review-person availability, visibility metadata on the ordinary review people response, unknown-person rejection, target-wins merge semantics, and that a saved collection retaining a now-hidden person reopens and reevaluates with the same person criterion and result.
