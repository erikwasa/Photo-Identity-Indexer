...
The corrective production contract is therefore stronger than the prompt alone: Photo Identity must normalize model output to one complete short sentence before guard evaluation and persistence. Raw extra prose or token-limit fragments must not become displayable caption text. WI-0128 remains in progress until this normalization is implemented and sampled.
