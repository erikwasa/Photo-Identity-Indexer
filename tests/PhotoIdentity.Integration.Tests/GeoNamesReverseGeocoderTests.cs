...
public sealed class GeoNamesReverseGeocoderTests
{
    ...
    [Fact]
    public async Task Administrative_fallback_no_result_is_terminal_and_distinguishable()
    {
        CapturingHandler handler = new(request => new HttpResponseMessage(HttpStatusCode.OK)
        {
            Content = new StringContent(
        ...
        Assert.Equal("administrative-no-result", response.ErrorCode);
        Assert.Contains("administrative geography", response.ErrorMessage, StringComparison.OrdinalIgnoreCase);
        Assert.Equal(2, response.ProviderRequestCount);
    }
