...
public sealed class ReviewProxyScaleValidationTests
{
    ...
    public async Task Archive_proxy_measure_allows_one_exact_profile_for_scale_validation()
    {
        ...
        try
        {
            ...
            Assert.True(File.Exists(Path.Combine(outputRoot, "selected-profile", "one.jpg.proxy.jpg")));
        }
        finally
        {
    ...
    [Fact]
    public async Task Archive_proxy_measure_keeps_same_stem_different_source_extensions_distinct()
    {
        string directory = Path.Combine(
            Path.GetTempPath(),
            "PhotoIdentity.Integration.Tests",
        ...
        try
        {
            ...
            string outputRoot = Path.Combine(directory, "proxies");
            Directory.CreateDirectory(sourceRoot);
            using Mat image = new(new Size(800, 600), MatType.CV_8UC3, new Scalar(10, 60, 130));
            Cv2.ImEncode(".jpg", image, out byte[] jpeg);
            Cv2.ImEncode(".png", image, out byte[] png);
            await File.WriteAllBytesAsync(Path.Combine(sourceRoot, "same.jpg"), jpeg);
            await File.WriteAllBytesAsync(Path.Combine(sourceRoot, "same.png"), png);
            ...
            Assert.Equal(0, exitCode);
            Assert.Equal(string.Empty, error.ToString());
            Assert.Contains("source-images: 2", output.ToString(), StringComparison.Ordinal);
            Assert.True(File.Exists(Path.Combine(outputRoot, "selected-profile", "same.jpg.proxy.jpg")));
            Assert.True(File.Exists(Path.Combine(outputRoot, "selected-profile", "same.png.proxy.jpg")));
        }
        finally
        {
            if (Directory.Exists(directory))
