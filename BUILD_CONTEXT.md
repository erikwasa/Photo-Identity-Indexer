...
WI-0101 currently has PostgreSQL schema/repositories for manual photo tags, manual photo people, capture metadata, extended metadata and metadata inspection through schema version 17. The latest local slices fix the schema marker and convert the photo-tag endpoint, manual photo-people endpoint mutations, metadata-inspection service constructor, source-verification service, original-access availability checks, bounded archive-analysis coverage/availability seams, archive API coverage entry points, archive advancement worker coverage flow, archive item-filter coverage read, person-audit endpoint, identity-match regeneration run/policy state, review suggestion endpoints, main review action mutations, suggestion-gallery action history, review target proxy fallback and collection review-proxy file resolution to Core persistence contracts while keeping SQLite as the pre-cutover runtime binding.
...
For the M24 thread, continue WI-0101 by neutralizing the remaining SQLite-only normal-runtime dependencies, starting with Places/automatic place writes and then collection/photo-detail/smart-collection/slideshow/detector surfaces. Existing archive source observation, availability, hydration, coverage and storage contracts are now useful seams; keep normal DI on SQLite until WI-0102 and do not create dual writes.
