...
For the M24 thread, continue WI-0101 by neutralizing the remaining SQLite-only normal-runtime dependencies, next moving to smart-collection query/snapshot evaluation, slideshow/detector and remaining archive worker/query surfaces. Existing archive source observation, availability, hydration, coverage and storage contracts are now useful seams; keep normal DI on SQLite until WI-0102 and do not create dual writes.
