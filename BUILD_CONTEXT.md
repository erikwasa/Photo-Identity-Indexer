...
For the M24 thread, continue WI-0101 by neutralizing the remaining SQLite-only normal-runtime dependencies, next migrating source scanning and hosted archive advancement, then remaining identity workers and provider startup. Existing archive source observation, availability, hydration, coverage, storage and status/query contracts are now useful seams; keep normal DI on SQLite until WI-0102 and do not create dual writes.
